...
// MAIN COMPONENT
// ==========================================
export default function OptionsTab({ leaseId }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const resOpts = await axiosInstance.get(`/lease/leases/${leaseId}/options?page=${page}&size=10`);
      setOptions(resOpts.data.content || []);
      setTotalPages(resOpts.data.totalPages || 0);
      setSelectedIds([]);
  ...
  // Tự động điền Diện tích (Involved Area) khi chọn Suite Code
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) setSelectedIds(options.map(o => o.opId));
    else setSelectedIds([]);
  };
  ...
  const openEditModal = (option) => {
    setFormData({ ...option });
    setModalConfig({ isOpen: true, mode: "EDIT", data: option });
  };
  ...
  const handleSave = async (addAnother = false) => {
    try {
      ...
      if (modalConfig.mode === "EDIT") {
        await axiosInstance.put(`/lease/leases/${leaseId}/options/${formData.opId}`, formData);
      } else {
        await axiosInstance.post(`/lease/leases/${leaseId}/options`, formData);
      }
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm(`Delete ${selectedIds.length} options?`)) return;

    setLoading(true);
    try {
      for (const id of selectedIds) {
        try {
          await axiosInstance.delete(`/lease/leases/${leaseId}/options/${id}`);
        } catch (err) {
            ...
            <thead className="sticky top-0 z-10 bg-[#F39C12] text-white shadow-sm">
              <tr>
                <th className="w-10 px-3 py-2 text-center border-b border-[#D68910]">
                  <input type="checkbox" checked={options.length > 0 && selectedIds.length === options.length} onChange={handleSelectAll} className="w-3.5 h-3.5 rounded" />
                </th>
                <th className="px-4 py-2 font-semibold tracking-wide border-b border-[#D68910]">Option ID</th>
                <th className="px-4 py-2 font-semibold tracking-wide border-b border-[#D68910]">Option Type</th>
                ...
                <th className="px-4 py-2 font-semibold tracking-wide border-b border-[#D68910]">Area Involved</th>
                <th className="px-4 py-2 font-semibold tracking-wide border-b border-[#D68910] text-center">Active</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {options.length === 0 && !loading ? (
                <tr><td colSpan={9} className="py-12 text-center text-gray-500 font-medium">No options found.</td></tr>
              ) : (
                options.map((opt) => {
                  const isSelected = selectedIds.includes(opt.opId);
                  return (
                    <tr key={opt.opId} onClick={() => openEditModal(opt)} className={`cursor-pointer transition-colors group ${isSelected ? "bg-blue-50" : "hover:bg-orange-50/50"}`}>
                      <td className="px-3 py-2 text-center border-r border-gray-50">
                        <input type="checkbox" checked={isSelected} onChange={(e) => handleSelectRow(e, opt.opId)} onClick={(e) => e.stopPropagation()} className="w-3.5 h-3.5 rounded" />
                      </td>
                      ...
                      <td className="px-4 py-2 text-gray-700 border-r border-gray-50">{opt.exercisedBy || "-"}</td>
                      <td className="px-4 py-2 text-gray-700 border-r border-gray-50">{opt.areaInvolved ? `${opt.areaInvolved} sqm` : "-"}</td>
                      <td className="px-4 py-2 text-center">
                        <input type="checkbox" checked={opt.active} readOnly className="w-3.5 h-3.5 rounded accent-blue-600 cursor-default" onClick={e => e.stopPropagation()} />
                      </td>
                    </tr>
                  );
                })
          ...
          <div className="bg-white w-[900px] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]">
            ...
            <div className="bg-[#EFB034] px-5 py-3.5 flex justify-between items-center border-b border-[#D68910]">
              <h2 className="text-[13px] font-bold uppercase tracking-wide text-white drop-shadow-sm">
                {modalConfig.mode === "ADD" ? "Add New Option" : `Edit Option`}
              </h2>
              ...
              <div className="flex gap-2.5 items-center">
                ...
                {modalConfig.mode === "EDIT" && (
                  <button onClick={() => handleDeleteSingle(formData.opId)} className="bg-white text-[#DE3B40] hover:bg-red-50 border border-transparent hover:border-red-200 px-5 py-1.5 rounded text-xs font-bold shadow-md transition-colors">
                    Delete
                  </button>
                )}
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                ...
                <div className="flex flex-col gap-5 border-r border-gray-100 pr-4">
                  <Input 
                    label="Option ID" 
                    value={formData.opId} 
                    onChange={v => setFormData({...formData, opId: v})} 
                  ...
                  <Textarea 
                    ...
                    onChange={v => setFormData({...formData, opDescription: v})} 
                  />
                  <Select 
                    label="Option Type" required 
                    value={formData.opType} 
                    onChange={v => setFormData({...formData, opType: v})}
                    options={[
                ...
                <div className="flex flex-col gap-5 pl-2">
                  ...
                  />
                  <Input 
                    type="number" label="Involved Area (sqm)" 
                    value={formData.areaInvolved} 
                    onChange={v => setFormData({...formData, areaInvolved: v})} 
                    disabled={true} 
                    placeholder="Auto-filled from Suite Code"
                  />
                  <Select 
                    label="Suite Code" 
                    value={formData.suiteId} 
                    onChange={handleSuiteChange}
                    options={availableSuites.map(su => ({ value: su.id || su.suiteId, label: `${su.suiteId || su.id} - ${su.name || ''}` }))}
                  />
