...
export default function VatCountryConsole() {
  ...
  const [filters, setFilters] = useState({ vatCountryId: "", countryName: "" });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      } catch (err) { console.warn("Failed to fetch Countries API."); }
  ...
  const handleSave = async () => {
    try {
      ...
      if (modal.mode === "ADD") await axiosInstance.post("/cost/vat-countries", payload);
      else await axiosInstance.put(`/cost/vat-countries/${editId}`, payload);
      fetchData(); setModal({ isOpen: false, mode: "ADD" });
    } catch (error) { alert(error.response?.data?.error || "Error saving data!"); }
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this VAT Country?")) return;
    try {
  ...
  const openEdit = (item) => {
    setFormData({ vatCountryId: item.vatCountryId, countryName: item.countryName, vatPercent: item.vatPercent });
    setEditId(item.id); setModal({ isOpen: true, mode: "EDIT" });
  };
  ...
  return (
    <div className="p-6 bg-gray-50 h-full flex flex-col min-h-screen animate-[fadeIn_0.2s_ease-out]">
      <div className="flex justify-between items-center mb-4 shrink-0">
        ...
        <button onClick={() => { setFormData(initialForm); setModal({ isOpen: true, mode: "ADD" }); }} className="bg-[#DE3B40] hover:bg-[#C11C22] text-white px-5 py-2 rounded text-sm font-bold shadow-sm transition-colors">
          + Add VAT Country
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-4 shrink-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          <div className="flex flex-col gap-1">
            ...
            <input type="text" value={filters.vatCountryId} onChange={e => setFilters({...filters, vatCountryId: e.target.value})} placeholder="Search ID..." className="border border-gray-300 rounded px-3 py-1.5 text-xs focus:outline-none focus:border-blue-500 w-full" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold text-gray-600 uppercase">Country Name</label>
            <input type="text" value={filters.countryName} onChange={e => setFilters({...filters, countryName: e.target.value})} placeholder="Search Name..." className="border border-gray-300 rounded px-3 py-1.5 text-xs focus:outline-none focus:border-blue-500 w-full" />
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center backdrop-blur-sm p-4">
          <div className="bg-white w-[450px] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[#EFB034] px-5 py-3.5 flex justify-between items-center border-b border-[#D68910]">
              <h2 className="text-[13px] font-bold uppercase tracking-wide text-white drop-shadow-sm">{modal.mode === "ADD" ? "Add VAT Country" : "Edit VAT Country"}</h2>
              <button onClick={() => setModal({ isOpen: false, mode: "ADD" })} className="text-white hover:text-red-100 transition-colors"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <div className="p-6 flex flex-col gap-5 bg-gray-50">
              <div className="flex flex-col gap-1 w-full bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                ...
                <input type="text" value={formData.vatCountryId} onChange={e => setFormData({...formData, vatCountryId: e.target.value})} disabled={modal.mode === "EDIT"} className="border border-gray-300 rounded px-3 py-2 text-[12px] focus:outline-none focus:border-blue-500 disabled:bg-gray-100 w-full" placeholder="e.g., VAT-VN" />
                ...
                <select value={formData.countryName} onChange={e => setFormData({...formData, countryName: e.target.value})} className="border border-gray-300 rounded px-3 py-2 text-[12px] focus:outline-none focus:border-blue-500 w-full bg-white cursor-pointer">
                  <option value="">-- Select Country --</option>
                  {countriesList.map((c, idx) => {
                    const name = c.countryName || c.name || c.id;
                    const code = c.id || c.countryId;
                    return <option key={code || idx} value={name}>{code ? `${code} - ${name}` : name}</option>;
                  })}
                </select>
                ...
                <input type="number" step="0.01" value={formData.vatPercent} onChange={e => setFormData({...formData, vatPercent: e.target.value})} className="border border-gray-300 rounded px-3 py-2 text-[12px] focus:outline-none focus:border-blue-500 w-full" placeholder="e.g., 8.5" />
              </div>
            </div>
            <div className="p-4 border-t border-gray-200 flex justify-end gap-3 bg-white">
              ...
              <button onClick={handleSave} disabled={!formData.vatCountryId || !formData.countryName || formData.vatPercent === ""} className="px-6 py-2 bg-[#D68910] text-white rounded text-xs font-bold hover:bg-[#B9770E] disabled:opacity-50 shadow-sm transition-colors">{modal.mode === "ADD" ? "Save" : "Update"}</button>
            </div>
          </div>
        </div>
      )}
