    ...
    <div className="p-[48px] font-['Inter'] bg-white min-h-full">
      ...
      <div className="flex gap-[40px] items-start">
          ...
          <div 
            ...
            className={`p-4 flex items-center gap-3 cursor-pointer transition-colors ${
              activeTab === 'information' 
                ? 'bg-[#FFEAEA] text-[#E32128] font-bold border-l-4 border-[#E32128]' 
                : 'text-[#565E6C] hover:bg-gray-100'
            }`}
          >
        ...
        <div className="flex-1 max-w-[850px]">
          {/* TAB: INFORMATION */}
          {activeTab === 'information' && (
            <div className="bg-white border border-[#DDE1E6] p-8 rounded-[8px] min-h-[400px]">
              <h2 className="text-[24px] font-bold text-[#323842] mb-6 border-b pb-4">Personal Information</h2>
              {loadingProfile ? (
                <div className="flex justify-center items-center py-20">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#E32128]"></div>
                </div>
              ) : profile ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                  ...
                  <DetailItem label="Role" value={profile.roleName} />
                  <DetailItem label="Title" value={profile.employeeTitle} />
                  <DetailItem label="Department" value={profile.department} />
                  <DetailItem label="Company" value={profile.company} />
                  <DetailItem label="Birthday" value={profile.birthday} />
                  <DetailItem label="Gender" value={profile.gender} />
                  <DetailItem label="Manager" value={profile.manager} />
                  <DetailItem label="VPA Site" value={profile.vpasite} />
                </div>
              ) : (
                <div className="text-center text-[#565E6C] py-10">Cannot load personal information.</div>
              )}
            </div>
          )}
...
const DetailItem = ({ label, value }) => (
  <div className="flex flex-col border-b border-gray-100 pb-2">
    <span className="text-[13px] font-bold text-[#565E6C] uppercase tracking-wider mb-1">{label}</span>
    <span className="text-[16px] text-[#323842] font-medium">{value || '-'}</span>
  </div>
);
